...
\textbf{Test Name} & Ngăn chặn SQL Injection trong username \\ \hline
\textbf{Priority} & Quan trọng \\ \hline
\textbf{Preconditions} & Ứng dụng đang chạy, đang ở trang login \\ \hline
\textbf{Test Steps} & 
\begin{enumerate}[leftmargin=*,noitemsep]
\item Vào trang login
\item Nhập username là câu truy vấn SQL injection
\item Nhập password
\item Click nút Login
\end{enumerate} \\ \hline
\textbf{Test Data} & Username: admin' OR '1'='1'-- \newline Password: bất kỳ \\ \hline
\textbf{Expected Result} & 
\begin{itemize}[leftmargin=*,noitemsep]
\item Hiển thị lỗi: ``Thông tin đăng nhập không chính xác''
\item KHÔNG tiết lộ thông tin lỗi database
\item Không bypass được authentication
\item Hệ thống không bị crash
\end{itemize} \\ \hline
\textbf{Actual Result} & (Để trống) \\ \hline
\textbf{Status} & Not Run \\ \hline
\end{tabular}
\caption{Test Case Ngăn chặn SQL Injection}
\end{table}
