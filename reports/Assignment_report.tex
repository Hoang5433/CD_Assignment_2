...
\hline
Nguyễn Thanh Phú & Trưởng nhóm & 26.66\% & Câu 1, viết báo cáo , cover câu 4  \\
\hline
Hoàng Đình Phú Quý  & Thành viên & 26.66\% & Câu 5, Mở rộng , cover câu 2, viết báo cáo \\
\hline
Nhan Chí Phong & Thành viên & 26.66\% & Câu 3 , viết báo cáo , cover team \\
\hline
...
\subsubsection{3.2.2 Test tích hợp Product components với API service}

% ---------------------------------------------------------------
\subsubsubsection{a) Test ProductTable component với API}

\begin{itemize}
    \item Thiết lập test environment cho ProductTable.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Thiết lập test environment cho ProductTable}]
describe("a) Test ProductTable component voi API", () => {
  let originalState;
...
\begin{itemize}
    \item Kiểm tra hiển thị đúng sản phẩm từ store.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Hiển thị đúng sản phẩm từ store}]
...
\begin{itemize}
    \item Thiết lập test environment cho ProductForm.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Thiết lập test environment cho ProductForm}]
...
\begin{itemize}
    \item Kiểm tra tạo sản phẩm mới thành công và cập nhật state.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Tạo sản phẩm mới thành công và cập nhật state}]
...
\begin{itemize}
    \item Kiểm tra tạo sản phẩm thất bại và không cập nhật state.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Tạo sản phẩm thất bại và không cập nhật state}]
test("Create product FAILED and does NOT update state", async () => {
  ...
  await waitFor(() => {
    expect(toast.error).toHaveBeenCalledWith("Them moi that bai", {
      description: expect.anything(),
    });
  });
...
\begin{itemize}
    \item Kiểm tra xử lý lỗi khi cập nhật sản phẩm.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Xử lý lỗi khi cập nhật sản phẩm}]
...
\begin{itemize}
    \item Thiết lập test environment cho ProductDetail.
\end{itemize}

\begin{lstlisting}[language=Java, caption={Thiết lập test environment cho ProductDetail}]
...
\begin{lstlisting}[language=Java, caption={Hiển thị đầy đủ thông tin chi tiết sản phẩm}]
...
\begin{itemize}
    \item Kiểm tra lấy danh sách sản phẩm với phân trang.
\end{itemize}

\begin{lstlisting}[language=Java, caption={GET /api/products - SUCCESS - Lấy danh sách sản phẩm (Mặc định)}]
@Test
@DisplayName("GET /api/products - SUCCESS - Lay danh sach san pham (Mac dinh)")
